...
def index(request):
    return render(
        request, "auctions/index.html", {"listings": Listing.objects.filter(sold=False)}
    )


def loginfxn(request):
...
def newListing(request):
    if request.method == "POST":
        ...
        return render(
            ...
            {"listing_form": CreateListingForm(), "categories": Category.objects.all()},
        )
...
def listing(request, listing_id):
    ...
    return render(
        ...
        "auctions/listing.html",
        {
            ...
            "category": category,
            "comments": Comment.objects.filter(listing=listing.id),
            "watching": WatchList.objects.filter(user=user, listing=listing).values(
                "watching"
            ),
            "is_owner": is_owner,
        },
...
def watchlist(request, user_id):
    listing_ids = WatchList.objects.filter(user=request.user, watching=True).values(
        "listing"
    )
...
def add_watchlist(request, listing_id):
    info = listing_info(request, listing_id)
    listing, user, is_owner, category, comments = (
        info[0],
        info[1],
        info[2],
        info[3],
...
def remove_watchlist(request, listing_id):
    info = listing_info(request, listing_id)
    listing, user, is_owner, category, comments, watch = (
        info[0],
        info[1],
        info[2],
        info[3],
